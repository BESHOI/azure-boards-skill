---
name: azure-boards
description: Azure DevOps Boards + full REST API — work items, WIQL, backlogs, sprints, Repos (Git/PRs), Pipelines (builds/releases), Test Plans, Artifacts. Alias of azure-devops. Works via PAT ($AZURE_DEVOPS_PAT / $AZURE_PAT). On first use, asks for project link + PAT, then performs Boards/Repos/Pipelines operations via curl.
version: 1.0
---
...
### Prerequisites — Agent must ask for link + key

**Before any read/create/update/delete**, if `AZURE_PAT` is not set or you don't know the project link, **ask the user first** (do not guess):

> "I need your Azure DevOps link and a PAT to continue. Please provide:
> 1) **Project link** — e.g. `https://dev.azure.com/{org}/{project}` or `https://{org}.visualstudio.com/{project}/_sprints/...` (I will parse `org`, `project`, `team`, `area`, `iteration` from it)
> 2) **PAT** — create at `https://dev.azure.com/{org}/_usersSettings/tokens` → `+ New Token` → Org `{org}` → Expiration **Custom 1 day** → Scopes **Work Items: Read & Write** (add `Code`, `Build`, etc. as needed) → Copy. Paste it here (I will store as `AZURE_PAT` env var, never log it)."

Then set:
```bash
export AZURE_PAT="paste"  # use `read -s AZURE_PAT` so it doesn't hit history
# verify
curl -s -u ":$AZURE_PAT" "https://dev.azure.com/{org}/_apis/projects?api-version=7.1" | head
```
Revoke at `/_usersSettings/tokens` after. For `curl` use `-u ":$PAT"` (Basic `base64(:PAT)` over HTTPS).

If the user already provided a link like `https://dev.azure.com/{org}/{project}/_sprints/taskboard/...`, parse `ORG`, `PROJECT`, `TEAM`, `AREA`, `ITERATION` from it and confirm back.
